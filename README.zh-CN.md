...
Technical Doc Parser 是一个面向 C++ 原生部署的文档解析基础设施，也可以理解为一个 backend-agnostic 的 Document Parsing Engine。
...
很多开源文档解析项目更关注最终转换效果：把文档转成 Markdown、HTML、JSON 或适合大模型/RAG 的 chunks。Technical Doc Parser 更关注底层解析引擎本身，让这些输出在 C++ 应用里稳定、可控、可扩展。
...
负责后端初始化和文档访问。当前实现通过 PDFium 打开 PDF，管理 PDFium 生命周期，读取页数和页级元数据，并把 PDFium 资源管理细节隔离在 PDF 模块内部。未来可以增加其他文档后端或外部解析器输出适配器。
...
cpp/
  ...
  document/     共享内部文档模型
  pdf/          PDFium-based PDF access, rendering, PDF text extraction
...
项目仍处于早期实现阶段。当前已经具备 C++17/CMake CLI、pinned PDFium setup、PDFium 生命周期管理、PDF 打开和页数读取、页面渲染、内部文本模型、PDF text layer 提取、OpenCV 图像预处理、可选 Tesseract OCR baseline、基础 layout analysis model、JSON manifest 输出，以及主流程冒烟测试。
...
后续实现应继续保持清晰 stage 边界，继续提升 layout detection 质量，再逐步加入 table structure recovery 和更稳定的 export/SDK layer。
