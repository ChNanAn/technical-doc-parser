...
预算内直接复用 PDFium 的 RGBA 像素，首次读图时才转成 BGR，从而省去 PNG 解码；
PNG 仍保留供展示和回退使用。可用 `--image-cache-bytes 0` 关闭，或指定其他字节上限；
预算按保留缓冲区的实际容量计算，不包含转换临时输出、渲染缓冲区、模型张量和当前未缓存页面。
详见[测量结果与限制](docs/optimization-2026-09.md)。
...
产物保留与自动清理已提供预览命令，默认关闭删除。模型调用的强制中断、原始上传文件的保留策略和跨存储灾难恢复仍待完善，详见[平台语义](platform/README.md)。
