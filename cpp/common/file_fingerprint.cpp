...
Status fingerprintFile(const std::filesystem::path& path, FileFingerprint& fingerprint) {
    ...
    if (!input) {
        return Status::error(
            "source.fingerprint_open_failed", "failed to open source for fingerprinting: " + path.string());
    }
    ...
    if (!input.eof()) {
        fingerprint = {};
        return Status::error(
            "source.fingerprint_read_failed", "failed while reading source for fingerprinting: " + path.string());
    }
