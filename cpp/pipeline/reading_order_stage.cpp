...
ReadingOrderStage::ReadingOrderStage(const reading_order::ReadingOrderService& reading_order)
...
common::Status ReadingOrderStage::order(const PipelineContext& context,
    ...
    for (std::size_t index = 0; index < pages.size(); ++index) {
        ...
        if (!reading_order_.order(pages[index], page_layouts[index], reading_order)) {
            return common::Status::error("reading_order.backend_failed",
                                         "reading order failed for page " + std::to_string(index + 1));
        }
        page_reading_orders.push_back(std::move(reading_order));
    }
